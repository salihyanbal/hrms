...
export default function LanguageUpdateModal({ trigger, candidateLanguage }) {
  const [open, setOpen] = useState(false);
  const [languages, setLanguages] = useState([]);
  let candidateLanguageService = new CandidateLanguageService();
  const deleteLanguage = (candidateLanguage) => {
  ...
  const formik = useFormik({
    initialValues: {
      language: {
        id: candidateLanguage.language.id,
      },
      level: candidateLanguage.level,
    },
    validationSchema: educationUpdateSchema,
    onSubmit: (values) => {
      candidateLanguage.language = { id: values.language.id };
      candidateLanguage.level = values.level;
      candidateLanguageService.save(candidateLanguage).then((result) => {
        //TODO: SAYFA YENILEME
      });
    },
  });
              ...
              <div style={{ float: "right" }}>
                <DeleteModal
                  ...
                  message={
                    ...
                    " dilini silmek istiyor musunuz?"
                  }
                  header="Eğitim sil"
                  onAcceptMethod={deleteLanguage}
                  methodParams={candidateLanguage}
                />
